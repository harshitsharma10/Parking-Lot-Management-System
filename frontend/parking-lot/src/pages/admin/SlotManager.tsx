...
export default function SlotManager() {
  ...
  const filtered = slots.filter(s => filter === 'ALL' || s.slot_type === filter)
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          ...
          <div key={stat.label} className="bg-white/3 border border-white/8 rounded-xl p-4">
            <p className="text-white/20 font-mono text-xs tracking-widest uppercase mb-2">{stat.label}</p>
            <p className={`text-3xl font-light ${stat.accent ? 'text-[#e8ff47]' : 'text-white'}`}>
              {stat.value}
            </p>
          </div>
        ))}
        ...
        {filtered.map(slot => (
          ...
          >
            ...
            <p className={`text-[10px] font-mono mt-2 uppercase tracking-widest
              ${slot.status === 'OCCUPIED' ? 'text-[#e8ff47]' : 'text-white/20'}`}>
              {slot.status}
            </p>
          </div>
        ))}
